...
const reducer = (state, action) => ({
  ...
  set_cachedMovie: {...state, clickedMovie: action.cachedMovie},
...
const useMovies = () => {
  ...
  const handleMovieClick = (movie) => {
    ...
    const cachedMovie = watchedMovies.filter(film => movie.imdbID === film.imdbID)[0]
    
    if(cachedMovie) {

      if (cachedMovie.Runtime === "N/A") {
        dispatch({type: "set_cachedMovie", cachedMovie: { ...cachedMovie, Runtime: "0 min" }})
      } else {
        dispatch({type: "set_cachedMovie", cachedMovie: cachedMovie})
      }

      dispatch({type: "set_rating", rating: movie.rate || 0})
      dispatch({type: "ended_fetch"})
      return
    }
